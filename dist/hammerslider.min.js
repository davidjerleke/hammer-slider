...
function HammerSlider(_this, options) {
  ...
  function getItemsAsArray(arrayLikeCollection) {
    var returnArray = [];
    forEachItem(arrayLikeCollection, function (item) {
  ...
  function setItemDistanceToFlip(itemArray) {
    return function (item) {
      var distanceToFlip = itemArray.reduce(function (accumulator, current) {
        return accumulator + current.width / item.width * 100;
      }, 0);
  ...
  function _setupSlider(startSlide) {
    ...
    // Gather calculations
    var setSlideWidth = setItemWidth(slider.container.offsetWidth);
    var setDistanceToSlide = setDistanceToItem(setItemAlignment(o.slideAlign));
    var setDistanceBetweenSlides = setDistanceBetweenItems(nrOfSlides - 1);

    // Store items & make calculations
    slideData = getItemsAsArray(slider.slides).map(function (slide) {
    ...
    if (!o.rewind) {
      ...
      var lastSlide = slideData[nrOfSlides - 1];
  ...
  function setup() {
    ...
    if (o.onSetup) setTimeout(function () {
