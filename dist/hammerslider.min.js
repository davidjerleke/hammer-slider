...
function HammerSlider(_this, options) {
  ...
  function getContentWidth(container, items) {
    return items.reduce(function (accumulator, _ref) {
      var offsetWidth = _ref.offsetWidth;
      return accumulator + getPercentageOfTotal(offsetWidth, container.offsetWidth);
    }, 0);
  }
  ...
  function setDistanceToItem() {
    return function (item, index, itemArray) {
      var distanceToThis = itemArray.reduce(function (accumulator, _ref2, innerIndex) {
  ...
  function setDistanceBetweenItems(lastItemIndex, contentWidth) {
    var infiniteDistance = function infiniteDistance(index) {
      return OPTIONS.infinite && !index ? contentWidth : 0;
    };
    return function (item, index, itemArray) {
      var nextItem = itemArray[!index ? lastItemIndex : index - 1];
      var distanceToNext = nextItem.distanceToThis - item.distanceToThis + infiniteDistance(index);
  ...
  function setItemDistanceToFlip(contentWidth) {
    return function (item, index, itemArray) {
      var distanceToFlip = contentWidth / item.width * 100;
  ...
  function getSlideData(container, items, lastItemIndex, contentWidth) {
    ...
    var setDistanceBetweenSlides = setDistanceBetweenItems(lastItemIndex, contentWidth);
    var setSlideDistanceToFlip = setItemDistanceToFlip(contentWidth);

    return [].concat(_toConsumableArray(items)).map(function (element) {
  ...
  function getFlipItems() {
    ...
    var toLeft = getItemsInFraction(leftGapWidth, -1, 0);
    var toRight = getItemsInFraction(rightGapWidth, 1, SLIDER.lastSlideIndex);

    /* CLEAN UP BELOW */
    var diff = SLIDER.slideData[toRight[toRight.length - 1]].width - SLIDER.slideData[toLeft[0]].width;

    if (Math.abs(diff) > 0 && SLIDER.lastSlideIndex > 2) {
      var leftGap = rightGapWidth - toRight.reduce(function (accumulator, item, index) {
        if (index === toRight.length - 1) return accumulator;
        return accumulator += SLIDER.slideData[item].width;
      }, 0);

      var lastGap = SLIDER.slideData[toRight[toRight.length - 1]].width - leftGap;
  ...
  function positionFlipItems(direction) {
    ...
    var getSlidePosition = getFlipItemPosition()[direction];

    SLIDER.flipData.items.all.forEach(function (itemIndex) {
  ...
  function getFlipPoint(itemArray, direction) {
    return function (flipIndex, intersectItem) {
      ...
      var flipPoint = getFlipDistance()[direction](intersectItem);
      var position = intersectItem.position;
      return !position ? flipPoint : flipPoint + SLIDER.contentWidth * (position < 0 ? 1 : -1);
    };
  }
  ...
  function flip(position, direction) {
    ...
    var currentFlip = flipData.state[direction];
    var oppositeFlip = flipData.state[direction * -1];
    var slideableDistance = flipData.slideableDistance[direction];
    var hasReached = hasReachedDistance(position)[direction];
    var setFlipPoint = getFlipPoint(flipData.items.all, direction);
    var flipPoint = setFlipPoint(currentFlip.flipIndex, slideData[currentFlip.intersectIndex]);
  ...
  function setupSlider(element, startIndex) {
    ...
    var contentWidth = getContentWidth(container, slides);
    var currentSlideIndex = startIndex;
    var lastSlideIndex = slides.length - 1;
    var isLastSlide = isLastItemIndex(lastSlideIndex);
    var slideData = getSlideData(container, slides, lastSlideIndex, contentWidth);
