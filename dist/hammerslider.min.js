...
function HammerSlider(_this, options) {
  ...
  function setSlideData(container, items, lastItemIndex) {
    ...
    }).map(setSlideWidth).map(setDistanceToSlide).map(setDistanceBetweenSlides).map(setSlideDistanceToFlip);
  ...
  function flip(position, direction) {
    var firstSlide = SLIDER.slideData[0];
    var slideBeforeLast = SLIDER.slideData[SLIDER.lastSlideIndex - 1];
    var lastSlide = SLIDER.slideData[SLIDER.lastSlideIndex];
    var align = setItemAlignment(OPTIONS.alignSlides);
