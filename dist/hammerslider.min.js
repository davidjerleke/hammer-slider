...
function HammerSlider(_this, options) {
  ...
  function setForwardFlips() {
    ...
    var backwardAccumulator = 0;
    var lastFoldWidth = firstSlide.distanceToThis + 1;
    var itemsBackward = [];
    var itemIndex = 0;

    flipData.itemsBackward = forEachItem(SLIDER.slides, function () {
    ...
    var itemBoundIndex = flipData.itemsBackward[flipData.itemsBackward.length - 2] || 0;
    var itemToFlipIndex = getNextItemIndex(itemBoundIndex, -1);

    slideData.forEach(function (item, index) {
      var position = index > SLIDER.lastSlideIndex - flipData.itemsBackward.length ? item.distanceToFlip * -1 : 0;
    ...
    SLIDER.flipData[1] = {
      itemBoundIndex: itemBoundIndex,
      itemToFlipIndex: itemToFlipIndex
    };
  ...
  function setBackwardFlips() {
    var slideData = SLIDER.slideData;
    var firstSlide = slideData[0];
    var lastSlide = slideData[SLIDER.lastSlideIndex];
    var flipData = {};

    var forwardAccumulator = 0;
    var firstFoldWidth = 100 - firstSlide.alignDistance;
    var itemsForward = [0]; // Remove 0 from here and add it later: SLIDER.flipData.itemsForward.concat(SLIDER.flipData.itemsBackward)
    var itemIndex = 0;

    flipData.itemsForward = forEachItem(SLIDER.slides, function () {
  ...
  function flip(position, direction) {
    ...
    if (direction === 1) {
      var _ret = function () {
        var currentFlip = SLIDER.flipData[direction];
        var boundItem = slideData[currentFlip.itemBoundIndex];
        var flipItem = slideData[currentFlip.itemToFlipIndex];

        if (position < -SLIDER.contentWidth + firstSlide.alignDistance + 1) {
          mergeObjects(currentFlip, SLIDER.flipData.resetForward);
          slideData.forEach(function (item, index) {
            var position = index > SLIDER.lastSlideIndex - SLIDER.flipData.itemsBackward.length ? item.distanceToFlip * -1 : 0;
            setTranslate(item.element, position);
            mergeObjects(item, { position: position });
          });
          return {
        ...
        var bound = function () {
          var boundItemArray = SLIDER.flipData.itemsForward.concat(SLIDER.flipData.itemsBackward);
      ...
      var _currentFlip = SLIDER.flipData[direction];
